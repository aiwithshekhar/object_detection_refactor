...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        # print (f'input tensor {p.shape}')
        if ONNX_EXPORT:
            bs = 1  # batch size
        else:
            bs, _, ny, nx = p.shape  # bs, 255, 13, 13
            # print (f'this is p {p.shape}')
            if (self.nx, self.ny) != (nx, ny):
                create_grids(self, img_size, (nx, ny), p.device, p.dtype)

        # p.view(bs, 255, 13, 13) -- > (bs, 3, 13, 13, 85)  # (bs, anchors, grid, grid, classes + xywh)
        p = p.view(bs, self.na, self.no, self.ny, self.nx).permute(0, 1, 3, 4, 2).contiguous()  # prediction
        # print(f'output tensor {p.shape}')
        if self.training:
            return p
            ...
            # print (f'mesh grid {self.grid_xy}')
            # print (f'ioshapes {io[0][0][0][0]}')
            # print (f'sigmoid {torch.sigmoid(io[0][0][0][0])}')
            # print (io.shape)
            io[..., :2] = torch.sigmoid(io[..., :2]) + self.grid_xy  # xy
            # print(f'values are {io[..., :2].shape}')
            # print (f'values are {torch.sigmoid(io[..., :2])}__{self.grid_xy}__{torch.sigmoid(io[..., :2]) + self.grid_xy}')
